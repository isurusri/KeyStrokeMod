...
public class MouseButton {
  ...
  public void renderMouseButton(final int x, final int y, final int textColor) {
    ...
    if (pressed) {
      ...
      this.textBrightness =
          Math.max(0.0, 1.0 - (System.currentTimeMillis() - this.lastPress) / 20.0);
    } else {
    ...
    Gui.drawRect(
        ...
        x + this.xOffset + 34,
        y + this.yOffset + 22,
        2013265920 + (this.color << 16) + (this.color << 8) + this.color);
    ...
    this.mc.fontRenderer.drawString(
        ...
        y + this.yOffset + 4,
        -16777216
            + ((int) (red * this.textBrightness) << 16)
            + ((int) (green * this.textBrightness) << 8)
            + (int) (blue * this.textBrightness));
    final String cpsText = this.getCPS() + " CPS";
    final int cpsTextWidth = this.mc.fontRenderer.getStringWidth(cpsText);
    GL11.glScalef(0.5f, 0.5f, 0.5f);
    this.mc.fontRenderer.drawString(
        cpsText,
        (x + this.xOffset + 17) * 2 - cpsTextWidth / 2,
        (y + this.yOffset + 14) * 2,
        -16777216
            + ((int) (255.0 * this.textBrightness) << 16)
            + ((int) (255.0 * this.textBrightness) << 8)
            + (int) (255.0 * this.textBrightness));
